...
%% API
-export([start/0, stop/0, start_link/2, set/3, set/4, get/2, get/3, del/2, del/3,
         preflist/2, preflist/3, get_state/1, get_nodes/1, join/2, leave/2, get_buckets/0]).
...
-spec start_link(atom(), options()) -> gen_server:start_ret().
...
set(Bucket, Key, Value, Opts) ->
    call(Bucket, Key, Opts, {set, Key, Value}).
...
preflist(Key, N, Nodes) ->
    ...
    PivotNode = toy_kv_jchash:calculate(CHashKey, length(SortedNodes)) + 1,
...
join(Bucket, Nodes) ->
    ...
    {OkRep, _} =
        global:trans({{?MODULE, Bucket}, self()},
...
handle_call({join, Nodes}, _From, #state{nodes = CurrentNodes} = State) ->
    ...
    NewNodesOk =
        lists:foldl(fun(E, Acc) -> net_adm:ping(E) =:= pong andalso Acc end, true, NewNodes),
    case NewNodesOk of
        true ->
...
handle_call(Request, From, State) ->
    error_logger:warning_msg("Unexpected message:\nhandle_call(~p, ~p, ~p)\n",
                             [Request, From, State]),
    {reply, ok, State}.
...
init_bucket(#state{bucket = Bucket, copies = Copies}) ->
    _ = mnesia:create_table(Bucket,
                            [{Copies, [node()]}, {attributes, tuple_to_list(?TOY_KV)}]),
    ok.
...
reconcile_values(OkNodes, _) ->
    F = fun ({_, {ok, _} = X}, {OkL, ErrL}) ->
                {[X | OkL], ErrL};
            ({_, {error, _} = X}, {OkL, ErrL}) ->
                {OkL, [X | ErrL]};
            ({_, ok}, {OkL, ErrL}) ->
                {[ok | OkL], ErrL};
            ({_, X}, {OkL, ErrL}) ->
...
get_replicas(Opts) ->
    case lists:keyfind(replicas, 1, Opts) of
        {_, N} ->
            N;
        _ ->
            application:get_env(toy_kv, replicas, 1)
    end.
