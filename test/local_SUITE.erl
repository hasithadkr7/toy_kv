...
%% Tests
-export([t_kv_ops/1, t_kv_disk_persistence/1]).
...
all() ->
    [t_kv_ops, t_kv_disk_persistence].

init_per_suite(Config) ->
    ok = application:set_env(toy_kv, mode, local),
    toy_kv:start(),
    Config.
...
t_kv_ops(_Config) ->
    lists:foreach(fun t_kv_ops_/1, [b1, b2]).
...
t_kv_ops_(Bucket) ->
    ...
    ct:print("\e[1;1m t_kv_ops Bucket: ~p \e[0m\e[32m[OK] \e[0m", [Bucket]),
    ok.

t_kv_disk_persistence(_Config) ->
    %% Set some K/V pairs
    Set = fun({B, K, V}) -> ok = toy_kv:set(B, K, V) end,
    lists:foreach(fun(B) -> [Set({B, X, X}) || X <- lists:seq(1, 3)] end, [b1, b2]),

    %% Check
    Get = fun({B, K, V}) -> {ok, V} = toy_kv:get(B, K) end,
    lists:foreach(fun(B) -> [Get({B, X, X}) || X <- lists:seq(1, 3)] end, [b1, b2]),
    ...
    ct:print("\e[1;1m t_kv_disk_persistence \e[0m\e[32m[OK] \e[0m"),
    ok.
